...
// is malformed.
//
// A go_glob (`**`) should appear surrounded by path separators such as
// `/**/`.  A mid-pattern go_glob (`**`) behaves like bash's globstar
...
// goGlob.ErrBadPattern, being equal to path.ErrBadPattern.
...
func doMatchWithSeparator(pattern, name string, separator rune, validate bool, caseInsensitive bool, goGlobPatternBacktrack, goGlobNameBacktrack, starPatternBacktrack, starNameBacktrack, patIdx, nameIdx int) (matched bool, err error) {
	patLen := len(pattern)
	nameLen := len(name)
	startOfSegment := true
MATCH:
	for nameIdx < nameLen {
		if patIdx < patLen {
			...
			case '*':
				if patIdx++; patIdx < patLen && pattern[patIdx] == '*' {
					// go_glob - must begin with a path separator, otherwise we'll
					...
					if startOfSegment {
						...
						// go_glob must also end with a path separator, otherwise we're
						// just going to treat the go_glob as a single star like bash
						patRune, patRuneLen := utf8.DecodeRuneInString(pattern[patIdx:])
						if patRune == separator {
							patIdx += patRuneLen

							goGlobPatternBacktrack = patIdx
							goGlobNameBacktrack = nameIdx
			...
			case '{':
				...
				for {
					...
					result, err := doMatchWithSeparator(pattern[:beforeIdx]+pattern[patIdx:commaIdx]+pattern[closingIdx+1:], name, separator, validate, caseInsensitive, goGlobPatternBacktrack, goGlobNameBacktrack, starPatternBacktrack, starNameBacktrack, beforeIdx, nameIdx)
					if result || err != nil {
						return result, err
					}
				...
				return doMatchWithSeparator(pattern[:beforeIdx]+pattern[patIdx:closingIdx]+pattern[closingIdx+1:], name, separator, validate, caseInsensitive, goGlobPatternBacktrack, goGlobNameBacktrack, starPatternBacktrack, starNameBacktrack, beforeIdx, nameIdx)
		...
		if goGlobPatternBacktrack >= 0 {
			// `**` backtrack, advance `name` past next separator
			nameIdx = goGlobNameBacktrack
			for nameIdx < nameLen {
				nameRune, nameRuneLen := utf8.DecodeRuneInString(name[nameIdx:])
				nameIdx += nameRuneLen
				if nameRune == separator {
					goGlobNameBacktrack = nameIdx
					patIdx = goGlobPatternBacktrack
					startOfSegment = true
					continue MATCH
				}
			}
